...
public class MapLoader {
    ...
    private Map<String, Road> buildRoads(MapConfig config, Map<String, MapConfig.NodeConfig> nodes) {
    ...
    private List<SpawnPoint> buildSpawnPoints(MapConfig config) {
        if (config.getSpawnPoints() == null) return List.of();
    ...
    private List<DespawnPoint> buildDespawnPoints(MapConfig config) {
        if (config.getDespawnPoints() == null) return List.of();
        return config.getDespawnPoints().stream()
            .map(dp -> new DespawnPoint(dp.getRoadId(), dp.getLaneIndex(), dp.getPosition()))
            .toList();
    }

    private Map<String, Intersection> buildIntersections(MapConfig config, Map<String, MapConfig.NodeConfig> nodes) {
        Map<String, Intersection> intersections = new LinkedHashMap<>();
        if (config.getIntersections() == null) return intersections;
    ...
    private void wireIntersectionConnections(MapConfig config, Map<String, Intersection> intersections) {
        for (MapConfig.RoadConfig rc : config.getRoads()) {
    ...
    private void buildTrafficLights(MapConfig config, Map<String, Intersection> intersections) {
        if (config.getIntersections() == null) return;

        for (MapConfig.IntersectionConfig ic : config.getIntersections()) {
            if (!"SIGNAL".equals(ic.getType()) || ic.getSignalPhases() == null) continue;
