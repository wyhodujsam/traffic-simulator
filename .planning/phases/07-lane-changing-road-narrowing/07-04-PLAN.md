...
phase: 7
plan: 4
name: "Lane Change & Road Narrowing Tests"
wave: 3
...
# Plan 7.4: Lane Change & Road Narrowing Tests

## Goal

Create a comprehensive test suite for the MOBIL lane-change engine and road narrowing functionality. Cover safety invariants, MOBIL correctness, two-phase conflict resolution, cooldown enforcement, forced lane changes, and end-to-end road narrowing pipeline.

## Tasks

### Task 1: Create LaneChangeEngineTest — MOBIL safety and correctness
...
- `backend/src/test/java/com/trafficsimulator/engine/PhysicsEngineTest.java` (reference test pattern)
</read_first>

<action>
Create `backend/src/test/java/com/trafficsimulator/engine/LaneChangeEngineTest.java`:
...
class LaneChangeEngineTest {
    ...
    // === Test 6: Forced lane change from closed lane ===
    @Test
    @DisplayName("Vehicle in closed lane merges to adjacent active lane (forced)")
    void forcedLaneChange() {
        Road road = createTwoLaneRoad();
        Lane lane0 = road.getLanes().get(0);
        Lane lane1 = road.getLanes().get(1);

        // Vehicle forced to change from lane 0
        ...
        // Vehicle should have moved to lane 1
...
<acceptance_criteria>
- `grep "class LaneChangeEngineTest" backend/src/test/java/com/trafficsimulator/engine/LaneChangeEngineTest.java` finds the class
- `grep "@Test" backend/src/test/java/com/trafficsimulator/engine/LaneChangeEngineTest.java | wc -l` shows 9 tests
- `grep "freeLanePreferred\|noChangeWhenAlone\|safetyCriterion\|cooldownEnforced\|inactiveLane\|forcedLaneChange\|conflictResolution\|noDualOccupancy\|laneChangeProgress" backend/src/test/java/com/trafficsimulator/engine/LaneChangeEngineTest.java` finds all test names
- `mvn test -f backend/pom.xml -Dtest=LaneChangeEngineTest` — all 9 tests pass
</acceptance_criteria>

### Task 2: Create RoadNarrowingIntegrationTest

<read_first>
- `backend/src/main/java/com/trafficsimulator/engine/SimulationEngine.java`
- `backend/src/main/java/com/trafficsimulator/engine/LaneChangeEngine.java`
- `backend/src/test/java/com/trafficsimulator/engine/SimulationEngineIntegrationTest.java` (reference pattern)
</read_first>

<action>
Create `backend/src/test/java/com/trafficsimulator/engine/RoadNarrowingIntegrationTest.java`:
...
class RoadNarrowingIntegrationTest {
    ...
    void forcedMergeCompletes() {
        ...
        // Place vehicles spaced out in lane 0
        Vehicle v1 = createVehicle("v1", 200, 15.0, lane0);
    ...
    void congestionFormsAfterClosure() {
        ...
        // Measure post-closure average speed (only active lane)
        double postAvg = lane1.getVehicles().stream()
            .mapToDouble(Vehicle::getSpeed)
            .average().orElse(0);

        // Post-closure speed should be lower due to increased density
        // (more vehicles in fewer lanes)
        assertThat(postAvg).as("Post-closure avg speed should be <= baseline")
...
<acceptance_criteria>
- `grep "class RoadNarrowingIntegrationTest" backend/src/test/java/com/trafficsimulator/engine/RoadNarrowingIntegrationTest.java` finds the class
- `grep "@Test" backend/src/test/java/com/trafficsimulator/engine/RoadNarrowingIntegrationTest.java | wc -l` shows 3 tests
- `mvn test -f backend/pom.xml -Dtest=RoadNarrowingIntegrationTest` — all 3 tests pass
- `mvn test -f backend/pom.xml` — all tests pass (no regressions)
</acceptance_criteria>
